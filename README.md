...
- Ghost State Scanner：`typescriptParser.ts` 在 `ts-morph` 原生路径中会扫描方法体 / 函数体，把未通过入参传入的隐式依赖追加到 `fission.demand`
...
- `treeSitterParser.ts`：统一 Tree-sitter AST 入口，负责多语言函数、类方法、参数、返回值抽取
...
- 当 TypeScript 项目显式切回 `native` 时，会额外启用 Ghost State Scanner，补齐方法体里的隐式状态依赖
