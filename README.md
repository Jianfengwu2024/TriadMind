...
- C++ 在 `tree-sitter` 路径下会启用 Ghost State Scanner，覆盖 class/struct 字段、全局对象、inline method 与顶层函数中的隐式状态访问
- Java 在 `tree-sitter` 路径下会启用 Ghost State Scanner，覆盖 `this.xxx`、类字段、静态字段与方法体内的隐式对象依赖
