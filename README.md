...
- Ghost State Scanner：`typescriptParser.ts` 与 `treeSitterParser.ts` 会扫描 TypeScript / JavaScript / Python 的方法体与函数体，把未通过入参传入的隐式依赖追加到 `fission.demand`
...
- `treeSitterParser.ts`：统一 Tree-sitter AST 入口，负责多语言函数、类方法、参数、返回值抽取，并为 TypeScript / JavaScript / Python 补充 Ghost State Scanner
